...
`.trim(),
  ...
  list_dlq_jobs: `
WHEN TO USE: Debug jobs that failed after all retries (Dead Letter Queue).
WHEN NOT: Normal monitoring — use list_jobs with status=failed.
RETURNS: { success, count, jobs[] }
PARAMS: limit, all_workspaces, workspace_path?
`.trim(),

  get_job_metrics: `
WHEN TO USE: Token/cost/duration estimates for a completed job.
WHEN NOT: For pending/running jobs (metrics not yet recorded).
RETURNS: { success, jobId, metrics: { promptTokens, completionTokens, estimatedCostUsd, durationMs } }
`.trim(),

  approve_job_step: `
WHEN TO USE: HITL — approve/reject agentic pipeline step paused at awaiting_approval.
WHEN NOT: Unless BRIDGE_HITL_ENABLED=1 and job status is awaiting_approval.
RETURNS: { success, jobId, approved, resumed? }
`.trim(),

  job_store_status: `
WHEN TO USE: Quick Supabase connectivity check before delegate_async/run_pipeline.
WHEN NOT: For full health — use bridge_status.
RETURNS: { configured, reachable, url, hint }
`.trim(),
...
`.trim(),
  ...
  get_session: `
WHEN TO USE: Retrieve session metadata and recent context pack items.
WHEN NOT: To execute new prompt — use continue_session.
RETURNS: { success, session, context[] }
`.trim(),

  list_sessions: `
WHEN TO USE: Find sessions for current workspace, optionally filter by provider.
WHEN NOT: When session_id is known — use get_session.
RETURNS: { success, count, sessions[] }
PARAMS: provider?, limit, workspace_path?
`.trim(),

  add_context: `
WHEN TO USE: Inject file contents, diffs, or notes into context pack for next continue_session.
WHEN NOT: As substitute for clear prompt instructions.
RETURNS: { success, context: { id, label, content } }
PARAMS: content, label?, session_id?, content_type, workspace_path?
`.trim(),
...
`.trim(),
  ...
  get_job_chunks: `
WHEN TO USE: Stream partial output of running async job.
WHEN NOT: For completed fast jobs.
RETURNS: { success, jobId, chunks: [{ seq, text, created_at }] }
PARAMS: job_id, since_seq, limit
`.trim(),

  get_health_history: `
WHEN TO USE: Historical provider health snapshots from Supabase.
WHEN NOT: For live status — use bridge_status.
RETURNS: { success, snapshots[] }
`.trim(),

  record_health_check: `
WHEN TO USE: Force health probe and persist snapshots for all providers.
WHEN NOT: During normal operations (bridge_status already probes).
RETURNS: { success, snapshots[] }
`.trim(),

  trigger_github_webhook: `
WHEN TO USE: Simulate GitHub push/PR/ping webhook to enqueue delegation job.
WHEN NOT: In production without testing intent.
RETURNS: { success, jobId, event, prompt }
PARAMS: event_type(push|pull_request|ping), payload, workspace_path?
`.trim(),

  realtime_worker_status: `
WHEN TO USE: Check/start/stop Supabase Realtime worker for auto job processing.
WHEN NOT: When using only synchronous tools.
RETURNS: { running, enabled, action?, hint }
PARAMS: action(status|start|stop)
NOTES: Requires BRIDGE_REALTIME_WORKER=1 at server start for auto-start.
`.trim(),

  verify_github_signature: `
WHEN TO USE: Debug HMAC X-Hub-Signature-256 validation for webhook payloads.
WHEN NOT: Normal agent operations.
