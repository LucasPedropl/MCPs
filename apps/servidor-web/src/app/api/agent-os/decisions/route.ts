...
export async function POST(request: NextRequest) {
  ...
  if (!row.topic || !row.chosen_option) {
    return NextResponse.json({ error: "topic e chosen_option são obrigatórios" }, { status: 400 });
  }

  const { data, error } = await db.from("agent_decisions").insert(row).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(request: NextRequest) {
  ...
  if (body.chosen_option !== undefined) updates.chosen_option = String(body.chosen_option);
  if (body.project !== undefined) updates.project = (body.project as string | null) || null;
  if (body.problem !== undefined) updates.problem = (body.problem as string | null) ?? null;
  if (body.rationale !== undefined) updates.rationale = (body.rationale as string | null) ?? null;
